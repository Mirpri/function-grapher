import pygame
from pygame.locals import *
import time
from tkinter import *
from tkinter.ttk import *
from sympy import *
from math import *
import _thread
import random
import base64
import os

black=(100,100,100)
white=(255,255,255)
#green=(0,155,0)
#pingreen=(100,200,100)
#yellow=(155,155,0)
#purple=(155,0,155)
viewpozy=0
viewpozx=0
gridspc=1
zm=-10
winsize=500

img = b''

root = Tk()
root.title("function grapher")
pygame.init()
pygame.display.set_caption('graph')
screen = pygame.display.set_mode((winsize,winsize))

tmp = open("tmp.ico","wb+")
tmp.write(base64.b64decode(img))
tmp.close()
root.iconbitmap("tmp.ico")
os.remove("tmp.ico")

funclist=[]
colist=[]
ran=random.randint(0,5)
tzb="0123456789abcdef"

def zoom(zm):
    global zoomx,zoomy,viewpozx,viewpozy,gridspc
    #print(zm)
    zm = 0.8**(float(zm))
    zm*=2
    if viewpozx!=0 or viewpozy!=0:
        viewpozx*=zoomx/zm*winsize
        viewpozy*=zoomx/zm*winsize
    zoomx = (zm/winsize)
    zoomy = (winsize/zm)
    ngridspc=10**(round(log(zoomx*25,10)))
    if ngridspc!=gridspc:
        gridspc=ngridspc
        gridlab.config(text="Grid side length: "+str(gridspc))
    print('zm',zm,gridspc)
    #clears()
    #drawall(0,0)
    #pygame.display.update()
zoom(zm)

def clears():
    global viewpozx,viewpozy,winsize
    screen.fill((white))
    #print('zomy',viewpozx+winsize/2,zoomy,int(viewpozx+winsize/2)%zoomy)
    i=(viewpozx+winsize/2)%(zoomy*gridspc)
    while i<=(viewpozx+winsize/2)%zoomy+(winsize):
        pygame.draw.rect(screen, (200,200,200), (int(i), 0, 1, winsize), 0)
        i+=zoomy*gridspc
    i=(viewpozy+winsize/2)%(zoomy*gridspc)
    while i<=(viewpozy+winsize/2)%zoomy+(winsize):
        pygame.draw.rect(screen, (200,200,200), (0,int(i), winsize,1 ), 0)
        i+=zoomy*gridspc
    i=(viewpozx+winsize/2)%(zoomy*gridspc*10)
    while i<=(viewpozx+winsize/2)%zoomy+(winsize):
        pygame.draw.rect(screen, (150,150,150), (int(i), 0, 2, winsize), 0)
        i+=zoomy*gridspc*10
    i=(viewpozy+winsize/2)%(zoomy*gridspc*10)
    while i<=(viewpozy+winsize/2)%zoomy+(winsize):
        pygame.draw.rect(screen, (150,150,150), (0,int(i), winsize,2 ), 0)
        i+=zoomy*gridspc*10
    pygame.draw.rect(screen, (black), (int(winsize/2-1+viewpozx), 0, 2, winsize), 0)
    pygame.draw.rect(screen, (black), (0, int(winsize/2-1+viewpozy), winsize, 2), 0)
    #pygame.display.update()
clears()
pygame.display.update()


fentry=Frame(root)
e1=Text(fentry,height=5,width=24,font=("Consolas",12,'bold'),bd=0)
e1.pack(side='left',fill=BOTH,expand=Y)
s = Scrollbar(fentry, orient=VERTICAL, command=e1.yview)
s.pack(fill=Y,expand=Y)
fentry.pack(expand=Y,fill=BOTH,side='left')
e1['yscrollcommand'] = s.set



def to16(c):
    ans="#"+tzb[int(c[0]/16)]+tzb[c[0]%16]+tzb[int(c[1]/16)]+tzb[c[1]%16]+tzb[int(c[2]/16)]+tzb[c[2]%16]
    return ans
def drawall(wait,in3d):
    global consa
    progress.config(mode='indeterminate')
    progress.start(20)
    for i in range(0, len(funclist)):
        if len(colist)<=i:
            r=150
            g=random.randint(0,150)
            b=0
            choo=(i+ran)%6
            e1.tag_add(str(i),float(i+1),float(i+1)+0.99999)
            if choo==0:
                colist.append((r,g,b))
            elif choo==4:
                colist.append((r,b,g))
            elif choo==2:
                colist.append((g,b,r))
            elif choo==3:
                colist.append((g,r,b))
            elif choo==1:
                colist.append((b,r,g))
            elif choo==5:
                colist.append((b,g,r))
        if funclist[i]=='':
            continue
        color=colist[i]
        e1.tag_delete(str(i))
        e1.tag_add(str(i),float(i+1),float(i+1)+0.99999)
        e1.tag_config(str(i),foreground = to16(colist[i]))
        if in3d:
            draw3D(funclist[i],consa,color)
        else:
            if funclist[i].find("=")!=-1:
                draw2(funclist[i],consa,wait,color)
            else:
                draw(funclist[i],consa,wait,color)
    progress.stop()
    progress.config(mode='determinate')
    progress['value']=0



def changea(a):
    global consa
    consa=round(float(a),1)
    #clears()
    #drawall(0,0)
    #pygame.display.update()
    la.config(text="a="+str(consa))

consa=0
#print(zoomx,zoomy)

def adddt(l,r,vl,vr,func,a,gap,dtmin,color):
    
    global zoomx,zoomy
    if abs(vr-vl)<=gap or r-l<dtmin:
        return
    if (vl>winsize and vr>winsize) or (vl<0 and vr<0):
        return
    m=(l+r)/2
    x=zoomx*(m-int(winsize/2)-viewpozx)
    #print(func,x,l,m,r,vl,vr)
    try:
        y=eval(func)
        if abs(y)>100000:
            return
        ym=int(winsize/2-zoomy*y+viewpozy)
        #print(l,m,r,vl,ym,vr)
        if ym>=0 and ym<=winsize:
            pygame.draw.circle(screen, (color), (int(m), ym), 1)
    except:
        ym=int(winsize/2+viewpozy)
    #print(l,m,r,vl,vr)
    adddt(l,m,vl,ym,func,a,gap,dtmin,color)
    adddt(m,r,ym,vr,func,a,gap,dtmin,color)


def draw(func,a,wait,color):
    global viewpozx,viewpozy

    lst=0
    if wait:
        gap=1
        dtmin=0.0000001
    else:
        gap=2
        dtmin=0.001
    #print(eval(func))
    for i in range(-int(winsize/2), int(winsize/2)+1):
        x = zoomx*(i-viewpozx)
        try:
            y = eval(func)
            trxx = int(i+int(winsize/2))
            tryy = int(int(winsize/2)-zoomy*y+viewpozy)
        except:
            adddt(i+int(winsize/2-viewpozx),i+int(winsize/2+1-viewpozx),lst,0,func,a,gap,dtmin,color)
            lst=0
            continue
        if tryy<=winsize and tryy>=0:
            pygame.draw.circle(screen, (color), (trxx, tryy), 1)
            if wait:
                root.update()
                pygame.display.update()
                #time.sleep(0.002)

        vl=lst
        vr=tryy
        #print(i,i+1,his[i],his[i+1])
        '''
        if vl>winsize and vr>winsize or vl<0 and vr<0:
            continue
        '''
        adddt(trxx-1,trxx,vl,vr,func,a,gap,dtmin,color)
        lst=tryy

    if wait:
        pygame.display.update()
'''
def draw3D(func,a,color):
    global viewpozx,viewpozy
    draw("x",0,0,(black))

    func=func+"+z*0.35355339"

    for j in range(-50,50):
        z=zoomx*(j*10-viewpozx)*2.828427124746
        #print(((func.replace("x","(x-z*0.35355339)")).replace("z",str(z))),a,color,z)
        draw(((func.replace("x","(x-z*0.35355339)")).replace("z",str(z))),a,0,color)
    for j in range(-50,50):
        z=zoomx*(j*10-viewpozx)
        #print((func.replace("x",str(z))).replace("z","((x-"+str(z)+")*2.828427124746)"),a,color,z)
        draw((func.replace("x",str(z))).replace("z","((x-"+str(z)+")*2.828427124746)"),a,0,color)
    
    pygame.display.update()
'''

def draw2(func,a,wait,color):
    global viewpozx,viewpozy
    x=Symbol('x')
    y=Symbol('y')
    sa=Symbol('a')
    if wait:
        gap=1
    else:
        gap=4  
    try:
        l,r=func.split('=')
        fs=solve((Eq(sympify(l),sympify(r))).subs(sa,a),y)
        print(fs)
    except:
        return 
    
    for f in fs:
        print(f)
        for i in range(-int(winsize/2/gap)-1, int(winsize/2/gap)+1):
            i*=gap
            vx = zoomx*(i-viewpozx)
            try:
                tryy = int(int(winsize/2)-zoomy*(f.subs(x,vx)).evalf()+viewpozy)
            except:
                #print(f)
                continue
            trxx = int(i+int(winsize/2))
            if tryy<=winsize and tryy>=0:
                pygame.draw.circle(screen, (color), (trxx, tryy), 1)
                if wait:
                    root.update()
                    pygame.display.update()
                    #time.sleep(0.002)
    try:
        fs=solve(Eq(sympify(l),sympify(r)).subs(sa,a),x)
        print(fs,type(fs))
    except:
        return
    for f in fs:
        print(f)
        for i in range(-int(winsize/2/gap)-1, int(winsize/2/gap)+1):
            i*=gap
            vy = (i+viewpozy)/zoomy
            try:
                trxx = int(int(winsize/2)+(f.subs(y,vy)).evalf()/zoomx+viewpozx)
            except:
                continue
            tryy = int(-i+int(winsize/2))
            if trxx<=winsize and trxx>=0:
                pygame.draw.circle(screen, (color), (trxx,tryy), 1)
                if wait:
                    root.update()
                    pygame.display.update()
                    #time.sleep(0.002)

        if wait:
            pygame.display.update()


def GO():
    global funclist
    funclistt = ((e1.get("0.0","end").replace(" ","")).replace("_","math.")).split("\n")
    clears()
    drawall(1,0)




running=1
def AF(x):
    global funclist,zoomx,consa,viewpozy,viewpozx
    viewpozyy=0
    viewpozxx=0
    zoomxx=zoomx
    consaa=consa
    while running==1:
        funclistt = ((e1.get("0.0","end").replace(" ","")).replace("_","math.")).split("\n")
        if funclist!=funclistt or viewpozx!=viewpozxx or viewpozy!=viewpozyy or zoomx!=zoomxx or consa!=consaa:
            funclist=funclistt
            viewpozyy=viewpozy
            viewpozxx=viewpozx
            zoomxx=zoomx
            consaa=consa
            clears()
            drawall(0,0)
            pygame.display.update()
        time.sleep(0.1)
    #print("exit")
_thread.start_new_thread( AF, (1,) )

def resize():
    global winsize
    winsize=int(e2.get())
    screen = pygame.display.set_mode((winsize,winsize),0,32)
    clears()
    drawall(0,0)
    pygame.display.update()


fwids=Frame(root)
bi=Button(fwids,text="Calibrate",command=GO)
bi.pack(fill=X)
'''
b3d=Button(fwids,text="3D(beta)",command=lambda:drawall(0,1))
b3d.pack(fill=X)
'''
progress=Progressbar(fwids)
progress.pack(fill=X,padx=2,pady=5)

fscale1=Frame(fwids)
Label(fscale1,text="zoom ").pack(side='left',fill=X)
s1 = Scale( fscale1,from_ = -20, to = 20,orient="horizontal",command=zoom)
s1.set(zm)
s1.pack(fill=X,pady=3)
fscale1.pack(fill=X)

fscale2=Frame(fwids)
la=Label(fscale2,text="a=0.000",width=8)
la.pack(side='left',fill=X)
s2 = Scale(fscale2, from_=-10, to=10, orient="horizontal", command=changea)
s2.set(0)
s2.pack(fill=X,pady=3)
fscale2.pack(fill=X)

fsize=Frame(fwids)
Label(fsize,text="Size:").pack(side='left',fill=X)
e2=Spinbox(fsize,width=6,from_=100,to=1000)
e2.pack(side='left')
e2.insert(0,winsize)
bi=Button(fsize,text="resize",command=resize)
bi.pack(side='right')
fsize.pack()
gridlab=Label(fwids,text="Grid side length: "+str(gridspc))
gridlab.pack()
fwids.pack(fill=BOTH,padx=10,pady=5)



def pygamepro(x):
    global running,viewpozx,viewpozy,zoomx,zoomy
    print(1)
    hold=0
    while running:
        time.sleep(0.1)
        for event in pygame.event.get():
                if event.type == QUIT:
                    print(1)
                    root.destroy()
                    exit()
                m=pygame.mouse.get_pressed()
                #print(m[0])
                if m[0]==1:
                    print(hold)
                    mx,my=pygame.mouse.get_pos()
                    if hold==1:
                        viewpozy+=my-lastpoy
                        viewpozx+=mx-lastpox
                        print(viewpozx,viewpozy)
                        
                    hold=1
                    lastpox=mx
                    lastpoy=my
                else:
                    hold=0
                if event.type==MOUSEBUTTONUP:
                    if event.button==4:
                        zm=s1.get()+1
                        s1.set(zm)
                        zoom(zm)
                        print(zm)
                    elif event.button==5:
                        zm=s1.get()-1
                        s1.set(zm)
                        zoom(zm)
                        print(zm)
_thread.start_new_thread( pygamepro, (1,) )
def on_closing():
    global running
    running=0
    time.sleep(0.1)
    root.destroy()
    exit()
root.protocol('WM_DELETE_WINDOW', on_closing)
root.mainloop()


#(a*x-1)/(a*x**2+4*a*x+a)
#math.sin(a/x)

#pyinstaller -F D:\vscode\fgf\FGF1.1.py -w -i D:\vscode\fgf\icon.ico
